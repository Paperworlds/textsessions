...
- feat: `ts new --git-profile` — decouple git commit identity from Claude profile
- feat: `GitProfile` dataclass + `git_profiles` / `default_git_profiles` in config.toml
- feat: per-identity gitconfigs written to `~/.config/textsessions/git-profiles/`; `GIT_CONFIG_GLOBAL` overrides textaccounts injection when set

## v0.13.0

- feat: per-session checkpoint log (textsessions-checkpoint v0.1.0)
- feat: draft textsessions-checkpoint spec v0.1.0
