...
class Config:
    ...
    git_profiles: list[GitProfile] = field(default_factory=list)
    default_git_profiles: dict[str, str] = field(default_factory=dict)
...
def load() -> Config:
    ...
    git_profiles = []
    for gp in data.get("git_profiles", []):
        git_profiles.append(GitProfile(
            name=gp["name"],
            display_name=gp["display_name"],
            email=gp["email"],
            signing_key=gp.get("signing_key", ""),
        ))
    default_git_profiles = dict(data.get("default_git_profiles", {}))
    ...
    return Config(
        ...
        git_profiles=git_profiles,
        default_git_profiles=default_git_profiles,
    )
...
def save(config: Config) -> None:
    ...
    if config.git_profiles:
        data["git_profiles"] = [
            {k: v for k, v in {
                "name": gp.name,
                "display_name": gp.display_name,
                "email": gp.email,
                **( {"signing_key": gp.signing_key} if gp.signing_key else {}),
            }.items()}
            for gp in config.git_profiles
        ]
    if config.default_git_profiles:
        data["default_git_profiles"] = dict(config.default_git_profiles)
