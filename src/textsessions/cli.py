...
def _complete_git_profiles(ctx: click.Context, param: click.Parameter, incomplete: str) -> list[click.shell_completion.CompletionItem]:
    try:
        config = load()
        return [
            click.shell_completion.CompletionItem(gp.name)
            for gp in config.git_profiles
            if incomplete.lower() in gp.name.lower()
        ]
    except Exception:
        pass
    return []
...
def new_cmd(repo_label: str, profile: str, name: str, priority: str | None, model: str, git_profile_name: str) -> None:
    ...
    # Resolve git profile: flag → per-Claude-profile default → None
    resolved_git_profile = None
    gp_name = git_profile_name or config.default_git_profiles.get(profile, "")
    if gp_name:
        by_name = {gp.name: gp for gp in config.git_profiles}
        if gp_name not in by_name:
            _die(f"Git profile '{gp_name}' not found in config. Available: {', '.join(by_name) or '(none)'}")
        resolved_git_profile = by_name[gp_name]
    ...
    # Prepend any env vars that fish's config.fish might override.
    fish_prefix = ""
    if "ANTHROPIC_BASE_URL" in env:
        fish_prefix += f"set -x ANTHROPIC_BASE_URL {shlex.quote(env['ANTHROPIC_BASE_URL'])}; "
    if "CLAUDE_CODE_OAUTH_TOKEN" in env:
        fish_prefix += f"set -x CLAUDE_CODE_OAUTH_TOKEN {shlex.quote(env['CLAUDE_CODE_OAUTH_TOKEN'])}; "
...
@click.option("--pinned", "pinned_only", is_flag=True, help="Only show pinned sessions")
def sessions_cmd(query: str, tag: str, profile: str, repo: str, use_cwd: bool, by_priority: bool, limit: int,
                 ...
                 resume_name: str, names_only: bool, reindex: bool, pinned_only: bool) -> None:
    ...
    filtered = filter_sessions(all_sessions, query=query, tag=tag, profile=profile, repo_label=repo,
                               ...
                               persona=persona, label=label, pinned_only=pinned_only)
    if by_priority:
    ...
    show_pin_col = any(s.pinned for s in filtered)
